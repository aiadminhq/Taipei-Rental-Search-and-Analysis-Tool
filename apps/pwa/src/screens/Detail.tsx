...
export function DetailScreen({ id }: { id: string }) {
  // `null` is the loading sentinel: liveQuery has not emitted yet. Seeding with `undefined`
  // (the "no such row" value) flashed 找不到房源 for a frame on every open.
  const l = useLive<Listing | undefined | null>(() => db.listings.get(id), [id], null);
  ...
  const doEnrich = async () => {
    ...
    // spec §10: an unreachable endpoint is not an error the user has to act on — 'pending'
    // gets a neutral note, only a real endpoint-side failure is surfaced in red.
    showToast(r === 'done' ? '補抓完成' : r === 'failed' ? '補抓失敗' : '已標記待補抓', r === 'done' ? 'success' : r === 'failed' ? 'error' : 'info');
  };

  const fact = (label: string, value: string, input?: JSX.Element) => (
    ...
    <main class="mx-auto max-w-lg pb-40">
      ...
      <section class="p-4">
        ...
        <dl class="grid grid-cols-2 gap-2">
          {editing && fact('租金', formatRent(l.rent), <input aria-label="租金" type="number" class="w-full rounded border px-1 dark:bg-gray-900" value={d.rent ?? ''} onInput={(e) => setDraft({ ...draft, rent: num((e.target as HTMLInputElement).value) })} />)}
          {fact('押金（月）', l.depositMonths?.toString() ?? '—', <input aria-label="押金" type="number" class="w-full rounded border px-1 dark:bg-gray-900" value={d.depositMonths ?? ''} onInput={(e) => setDraft({ ...draft, depositMonths: num((e.target as HTMLInputElement).value) })} />)}
          {fact('管理費', l.managementFee?.toString() ?? '—', <input aria-label="管理費" type="number" class="w-full rounded border px-1 dark:bg-gray-900" value={d.managementFee ?? ''} onInput={(e) => setDraft({ ...draft, managementFee: num((e.target as HTMLInputElement).value) })} />)}
          {fact('房型', l.roomType, <select aria-label="房型" class="w-full rounded border dark:bg-gray-900" value={d.roomType} onChange={(e) => setDraft({ ...draft, roomType: (e.target as HTMLSelectElement).value as RoomType })}>{RoomTypeSchema.options.map((o) => <option key={o}>{o}</option>)}</select>)}
          {fact('坪數', formatArea(l.areaPing), <input aria-label="坪數" type="number" step="0.5" class="w-full rounded border px-1 dark:bg-gray-900" value={d.areaPing ?? ''} onInput={(e) => setDraft({ ...draft, areaPing: num((e.target as HTMLInputElement).value) })} />)}
          {fact('樓層', l.floor ?? '—', <input aria-label="樓層" class="w-full rounded border px-1 dark:bg-gray-900" value={d.floor ?? ''} onInput={(e) => setDraft({ ...draft, floor: (e.target as HTMLInputElement).value || undefined })} />)}
          {fact('區', l.district ?? '—', <select aria-label="區" class="w-full rounded border dark:bg-gray-900" value={d.district ?? ''} onChange={(e) => setDraft({ ...draft, district: (e.target as HTMLSelectElement).value || undefined })}><option value="">—</option>{ALL_DISTRICTS.map((o) => <option key={o}>{o}</option>)}</select>)}
          {fact('地址', l.address ?? '—', <input aria-label="地址" class="w-full rounded border px-1 dark:bg-gray-900" value={d.address ?? ''} onInput={(e) => setDraft({ ...draft, address: (e.target as HTMLInputElement).value || undefined })} />)}
          {fact('捷運', l.mrtNearest ? `${l.mrtNearest}${l.mrtWalkMin !== undefined ? ` · ${l.mrtWalkMin} 分` : ''}` : '—', <input aria-label="捷運站" class="w-full rounded border px-1 dark:bg-gray-900" value={d.mrtNearest ?? ''} onInput={(e) => setDraft({ ...draft, mrtNearest: (e.target as HTMLInputElement).value || undefined })} />)}
          {fact('步行分', l.mrtWalkMin?.toString() ?? '—', <input aria-label="步行分" type="number" class="w-full rounded border px-1 dark:bg-gray-900" value={d.mrtWalkMin ?? ''} onInput={(e) => setDraft({ ...draft, mrtWalkMin: num((e.target as HTMLInputElement).value) })} />)}
          {fact('寵物', PET_LABEL[l.petPolicy ?? 'unknown'], <select aria-label="寵物" class="w-full rounded border dark:bg-gray-900" value={d.petPolicy ?? 'unknown'} onChange={(e) => setDraft({ ...draft, petPolicy: (e.target as HTMLSelectElement).value as PetPolicy })}>{PetPolicySchema.options.map((o) => <option key={o} value={o}>{PET_LABEL[o]}</option>)}</select>)}
          {fact('可入住', l.availableFrom ?? '—', <input aria-label="可入住" type="date" class="w-full rounded border px-1 dark:bg-gray-900" value={d.availableFrom ?? ''} onInput={(e) => setDraft({ ...draft, availableFrom: (e.target as HTMLInputElement).value || undefined })} />)}
        </dl>
      </section>
