...
class NotionService {
  ...
  async createRentalPage(pageData) {
    try {
      if (process.env.DISABLE_NOTION_WRITE === 'true') {
        const title = pageData.properties?.['房源名稱']?.title?.[0]?.text?.content || 'Unknown';
        this.logger.warn(`DISABLE_NOTION_WRITE is true, skipping Notion page creation for: ${title}`);
        return { id: 'mock-id-' + Date.now(), url: 'mock-url' }; // Return a mock response
      }
  ...
  formatRentalForDisplay(page) {
    ...
    // 提取圖片 URL
    let imageUrl = '';
    const photos = properties['照片']?.files;
    if (photos && photos.length > 0) {
      imageUrl = photos[0].file?.url || photos[0].external?.url || '';
    }

    return {
      id: page.id,
      title: properties['房源名稱']?.title?.[0]?.text?.content || 'Unknown',
      price: properties['租金']?.number || 0, // 重新命名為 price
      ...
      location: (properties['區域名稱']?.rich_text?.[0]?.text?.content || '') + (properties['地址']?.rich_text?.[0]?.text?.content || ''), // 組合 location
      city: properties['市區名稱']?.rich_text?.[0]?.text?.content || '',
      suitability: properties['適合度']?.rich_text?.[0]?.text?.content || '',
      score: properties['平均評分']?.number || 0, // 重新命名為 score
      advantages: properties['重要優勢']?.rich_text?.[0]?.text?.content || '',
      status: properties['看房狀態']?.select?.name || 'Unknown',
      tags: properties['設備與特色']?.multi_select?.map(item => item.name) || [], // 映射為 tags
      ...
      image: imageUrl, // 加入 image 屬性
      area: (properties['坪數']?.number ? properties['坪數'].number + '坪' : 'N/A'), // 格式化 area
  ...
  async getProperties() {
    try {
      ...
      return allRentals.map(rental => this.formatRentalForDisplay(rental));
    } catch (error) {
      this.logger.error('獲取房源資料失敗', { error: error.message });
      throw error;
    }
  }
